...
abstract class laf {
    ...
    static daf a(Class cls) {
        String str;
        ClassLoader classLoader = laf.class.getClassLoader();
        if (cls.equals(daf.class)) {
            str = b;
        } else {
            if (!cls.getPackage().equals(laf.class.getPackage())) {
        ...
        try {
            ...
            Iterator it2 = ServiceLoader.load(laf.class, classLoader).iterator();
            ...
            if (arrayList.size() == 1) {
                return (daf) arrayList.get(0);
            }
            if (arrayList.size() == 0) {
                return null;
            }
            try {
                return (daf) cls.getMethod("combine", Collection.class).invoke(null, arrayList);
