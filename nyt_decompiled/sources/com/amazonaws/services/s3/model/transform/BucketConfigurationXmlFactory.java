...
public class BucketConfigurationXmlFactory {
    ...
    private void addEventsAndFilterCriteria(XmlWriter xmlWriter, NotificationConfiguration notificationConfiguration) {
        Iterator<String> it2 = notificationConfiguration.getEvents().iterator();
        while (it2.hasNext()) {
            xmlWriter.start("Event").value(it2.next()).end();
        }
    ...
    private void addInventoryOptionalFields(XmlWriter xmlWriter, List<String> list) {
        ...
        while (it2.hasNext()) {
            xmlWriter.start("Field").value(it2.next()).end();
        }
        xmlWriter.end();
    }
    ...
    private void writeRule(XmlWriter xmlWriter, CORSRule cORSRule) {
        ...
        if (cORSRule.getAllowedMethods() != null) {
            Iterator<CORSRule.AllowedMethods> it3 = cORSRule.getAllowedMethods().iterator();
            while (it3.hasNext()) {
                xmlWriter.start("AllowedMethod").value(it3.next().toString()).end();
            }
        }
        ...
        if (cORSRule.getAllowedHeaders() != null) {
            Iterator<String> it5 = cORSRule.getAllowedHeaders().iterator();
            while (it5.hasNext()) {
                xmlWriter.start("AllowedHeader").value(it5.next()).end();
            }
        }
        xmlWriter.end();
    }
