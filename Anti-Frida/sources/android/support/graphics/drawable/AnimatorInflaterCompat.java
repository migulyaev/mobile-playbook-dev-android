...
public class AnimatorInflaterCompat {
    ...
    /* JADX INFO: Multiple debug info for r9v8 android.support.graphics.drawable.AnimatorInflaterCompat$PathDataEvaluator: [D('tvTo' android.util.TypedValue), D('evaluator' android.animation.TypeEvaluator)] */
    private static PropertyValuesHolder getPVH(TypedArray styledAttributes, int valueType, int valueFromId, int valueToId, String propertyName) {
        ...
        if (valueType2 == 2) {
            ...
            if (nodesFrom == null && nodesTo == null) {
                ...
                if (nodesTo == null) {
                    toType = toType2;
                    propertyValuesHolder3 = PropertyValuesHolder.ofObject(propertyName, evaluator, nodesFrom);
                } else if (!PathParser.canMorph(nodesFrom, nodesTo)) {
                    throw new InflateException(" Can't morph from " + fromString + " to " + toString);
                } else {
                    toType = toType2;
                    propertyValuesHolder3 = PropertyValuesHolder.ofObject(propertyName, evaluator, nodesFrom, nodesTo);
                }
            ...
            if (getFloats) {
                if (hasFrom) {
                    ...
                    if (hasTo) {
                        ...
                        returnValue = PropertyValuesHolder.ofFloat(propertyName, valueFrom2, valueTo6);
                    } else {
                        propertyValuesHolder = PropertyValuesHolder.ofFloat(propertyName, valueFrom2);
                    }
                } else {
                    if (toType2 == 5) {
                    ...
                    propertyValuesHolder = PropertyValuesHolder.ofFloat(propertyName, valueTo5);
                }
                ...
                if (hasTo) {
                    if (toType2 == 5) {
                        valueTo3 = (int) styledAttributes.getDimension(valueToId, 0.0f);
                        valueTo4 = 0;
                    } else if (isColorType(toType2)) {
                        valueTo4 = 0;
                        valueTo3 = styledAttributes.getColor(valueToId, 0);
                    } else {
                        valueTo4 = 0;
                        valueTo3 = styledAttributes.getInt(valueToId, 0);
                    }
                    int[] iArr = new int[2];
                    iArr[valueTo4] = valueFrom;
                    iArr[1] = valueTo3;
    ...
    private static void setupPathMotion(Path path, ObjectAnimator oa, float precision, String propertyXName, String propertyYName) {
        ...
        while (contourIndex2 < numPoints) {
            ...
            contourIndex2++;
            measureForTotalLength = measureForTotalLength;
        }
    ...
    /* JADX WARN: Code restructure failed: missing block: B:45:0x0111, code lost:
        if (r3.hasNext() == false) goto L_0x0120;
     */
