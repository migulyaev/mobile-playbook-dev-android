...
public class ConstraintTableLayout extends ConstraintWidgetContainer {
        ...
        if (count != 0) {
            ...
            if (system == this.mSystem) {
                ...
                for (int i3 = 0; i3 < count; i3++) {
                    ((ConstraintWidget) this.mChildren.get(i3)).addToSolver(system);
                }
            }
        }
    }
    ...
    public void updateFromSolver(LinearSystem system) {
        ...
        if (system == this.mSystem) {
            int num = this.mVerticalGuidelines.size();
            for (int i = 0; i < num; i++) {
                this.mVerticalGuidelines.get(i).updateFromSolver(system);
            }
            int num2 = this.mHorizontalGuidelines.size();
            for (int i2 = 0; i2 < num2; i2++) {
                this.mHorizontalGuidelines.get(i2).updateFromSolver(system);
            }
        }
    }
