...
public final class CommonUtils {
    ...
    public static byte[] fromHexadecimal(String message) {
        ...
        try {
            ...
            for (int i = 0; i < message.length(); i += 2) {
                ...
                int first = Integer.parseInt(stringBuffer.toString(), 16);
                StringBuffer stringBuffer2 = new StringBuffer();
                stringBuffer2.append("");
                stringBuffer2.append(message.charAt(i + 1));
                int second = Integer.parseInt(stringBuffer2.toString(), 16);
                result[i / 2] = (byte) (((first & 255) << 4) + 0 + (second & 255));
            }
            return result;
        } catch (Exception e) {
            throw new EncryptionOperationNotPossibleException();
    ...
    public static String substringAfter(String string, String separator) {
        ...
        if (separator == null || (pos = string.indexOf(separator)) == -1) {
            return "";
        }
        return string.substring(separator.length() + pos);
    }
