...
public final class HalfSerializer {
    ...
    public static <T> void onNext(Subscriber<? super T> subscriber, T t, AtomicInteger wip, AtomicThrowable error) {
    ...
    /* JADX WARN: Multi-variable type inference failed */
    public static <T> void onNext(Observer<? super T> observer, T t, AtomicInteger wip, AtomicThrowable error) {
        if (wip.get() == 0 && wip.compareAndSet(0, 1)) {
            observer.onNext(t);
            if (wip.decrementAndGet() != 0) {
                Throwable ex = error.terminate();
