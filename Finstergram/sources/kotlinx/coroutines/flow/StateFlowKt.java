...
public final class StateFlowKt {
    ...
    public static final void increment(MutableStateFlow<Integer> mutableStateFlow, int delta) {
        int current;
        do {
            current = mutableStateFlow.getValue().intValue();
        } while (!mutableStateFlow.compareAndSet(Integer.valueOf(current), Integer.valueOf(current + delta)));
    }
    ...
    public static final <T> Flow<T> fuseStateFlow(StateFlow<? extends T> stateFlow, CoroutineContext context, int capacity, BufferOverflow onBufferOverflow) {
        ...
        if (((capacity >= 0 && capacity <= 1) || capacity == -2) && onBufferOverflow == BufferOverflow.DROP_OLDEST) {
            return stateFlow;
        }
